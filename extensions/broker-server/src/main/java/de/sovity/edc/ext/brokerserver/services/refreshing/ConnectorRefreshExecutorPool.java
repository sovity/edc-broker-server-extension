...
public class ConnectorRefreshExecutorPool {
    ...
    public void execute() {
        // while connectorQueue is not empty and there are threads available
        while (!connectorQueue.isEmpty() && ((ThreadPoolExecutor) executorService).getActiveCount() < ((ThreadPoolExecutor) executorService).getMaximumPoolSize()) {
